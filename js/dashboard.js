...
function capturePhoto() {
    ...
    stopCamera();
    setupPreview(dataUrl);
}

// Setup Preview and Simulate Analysis
...
function simulateAnalysis() {
    ...
    // Simulate network request / AI inference time (2 to 4 seconds)
...
function showResults() {
    ...
    // Pick a random mock result for demonstration
    const randomResult = mockResults[Math.floor(Math.random() * mockResults.length)];
    
    // Update DOM
    diseaseNameEl.innerText = randomResult.name;
    document.querySelector('.result-section p').innerText = `Confidence: ${randomResult.confidence}`;
    
    const actionsList = document.querySelector('.result-section ul');
    actionsList.innerHTML = '';
    randomResult.actions.forEach(action => {
    ...
    if (randomResult.name === "Healthy Plant") {
